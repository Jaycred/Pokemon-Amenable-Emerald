...
EWRAM_DATA struct Pokemon reserveSlotMon = {0};
...
static u16 GetDeoxysStat(struct Pokemon *mon, s32 statId)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK_IN_BATTLE || GetMonData(mon, MON_DATA_SPECIES, NULL) != SPECIES_DEOXYS || FlagGet(FLAG_DEOXYS_NORMAL))
        return 0;
...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if(hasStab == 1)
    {
        //On STAB, check which attack stat is higher
        if(attack > spAttack) doPhysical = 1;
        else doPhysical = 0;
    }
...
u8 GiveMonToPlayer(struct Pokemon *mon)
{
    ...
    if (i >= PARTY_SIZE)
    {
        // Set mon in Reserve Slot if can't give
        CopyMon(&reserveSlotMon, mon, sizeof(*mon));
